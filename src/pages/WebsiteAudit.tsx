...
import { ArrowDown, CheckCircle2, ClipboardCheck, Compass, MessageCircle, Search } from 'lucide-react';
import { Contact } from '@/components/Contact';
import { Footer } from '@/components/Footer';
import { Navbar } from '@/components/Navbar';
import { WEBSITE_AUDIT_PROJECT_TYPE } from '@/data/contacts';
import { profile } from '@/data/profile';
import { websiteAuditStructuredData } from '@/data/structuredData';
import { websiteAuditPage } from '@/data/websiteAudit';
...
export default function WebsiteAudit() {
  useEffect(() => {
    applyDocumentMeta({
      ...
      title: websiteAuditPage.seo.title,
      description: websiteAuditPage.seo.description,
      ogTitle: websiteAuditPage.seo.ogTitle,
      ogDescription: websiteAuditPage.seo.ogDescription,
      ogType: websiteAuditPage.seo.ogType,
      ogLocale: profile.seo.ogLocale,
      ogSiteName: profile.brand.fullName,
      ogUrl: websiteAuditPage.seo.canonical,
      ogImage: profile.seo.ogImageUrl,
      canonical: websiteAuditPage.seo.canonical,
      twitterCard: profile.seo.twitterCard,
      twitterTitle: websiteAuditPage.seo.ogTitle,
      twitterDescription: websiteAuditPage.seo.ogDescription,
      twitterImage: profile.seo.ogImageUrl,
      themeColor: profile.seo.themeColor,
      robots: websiteAuditPage.seo.robots,
    });

    return applyDocumentStructuredData(websiteAuditStructuredData);
  }, []);
      ...
      <main>
        <section className="relative overflow-hidden pt-32 pb-18 sm:pt-36 sm:pb-24">
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-8">
            <section aria-labelledby="website-audit-title">
              <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/25 bg-indigo-500/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.22em] text-indigo-300 sm:mb-6">
                <Search size={14} />
                <span>{websiteAuditPage.hero.eyebrow}</span>
              </div>
              <h1
              ...
              >
                {websiteAuditPage.hero.title}
              </h1>
              <p className="mt-6 max-w-3xl text-base leading-relaxed text-slate-300 sm:text-lg">
                {websiteAuditPage.hero.description}
              </p>
              <p className="mt-5 max-w-2xl rounded-3xl border border-white/8 bg-slate-900/60 p-5 text-sm leading-relaxed text-slate-300 sm:text-base">
                {websiteAuditPage.hero.regionNote}
              </p>

              <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <a
                  href={websiteAuditPage.hero.primaryCtaHref}
                ...
                >
                  {websiteAuditPage.hero.primaryCta}
                </a>
                <a
                  href={websiteAuditPage.hero.secondaryCtaHref}
            ...
            <aside className="rounded-[2rem] border border-slate-800 bg-slate-900/55 p-5 shadow-2xl shadow-slate-950/40 backdrop-blur-xl sm:p-7 lg:rounded-[2.5rem]">
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="text-lg font-bold text-white">Проблемы, приоритеты и следующий шаг</h2>
                </div>
              </div>
              <ul className="space-y-3">
                {websiteAuditPage.deliverables.items.map((item, index) => (
        ...
        <section id={websiteAuditPage.situations.id} className="bg-slate-950 py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-9 max-w-3xl sm:mb-12">
              <h2 className="text-3xl font-bold text-white sm:text-4xl">
                {websiteAuditPage.situations.title}
              </h2>
              <p className="mt-4 text-base leading-relaxed text-slate-400 sm:text-lg">
                {websiteAuditPage.situations.description}
              </p>
            </div>
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-5">
              {websiteAuditPage.situations.items.map((item, index) => (
                <article
                  key={item.title}
        ...
        <section id={websiteAuditPage.auditAreas.id} className="relative overflow-hidden bg-slate-950 py-16 sm:py-20">
          <div className="absolute left-[-10rem] top-20 h-96 w-96 rounded-full bg-indigo-500/10 blur-[120px]" />
          <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-9 max-w-3xl sm:mb-12">
              <h2 className="text-3xl font-bold text-white sm:text-4xl">
                {websiteAuditPage.auditAreas.title}
              </h2>
              <p className="mt-4 text-base leading-relaxed text-slate-400 sm:text-lg">
                {websiteAuditPage.auditAreas.description}
              </p>
            </div>
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {websiteAuditPage.auditAreas.items.map((item) => (
        ...
        <section id={websiteAuditPage.process.id} className="bg-slate-950 py-16 sm:py-20">
          <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <h2 className="mb-10 text-3xl font-bold text-white sm:text-4xl">
              {websiteAuditPage.process.title}
            </h2>
            <ol className="space-y-4">
              {websiteAuditPage.process.items.map((item, index) => (
        ...
        <section id={websiteAuditPage.deliverables.id} className="relative overflow-hidden bg-slate-950 py-16 sm:py-20">
          <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-9 max-w-3xl sm:mb-12">
              <h2 className="text-3xl font-bold text-white sm:text-4xl">
                {websiteAuditPage.deliverables.title}
              </h2>
              <p className="mt-4 text-base leading-relaxed text-slate-400 sm:text-lg">
                {websiteAuditPage.deliverables.description}
              </p>
            </div>
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
              {websiteAuditPage.deliverables.items.map((item) => (
        ...
        <section id={websiteAuditPage.principles.id} className="bg-slate-950 py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="rounded-[2rem] border border-indigo-500/20 bg-indigo-500/10 p-6 sm:p-8 lg:p-10">
              <h2 className="max-w-3xl text-3xl font-bold text-white sm:text-4xl">
                {websiteAuditPage.principles.title}
              </h2>
              <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                {websiteAuditPage.principles.items.map((item) => (
        ...
        <section id={websiteAuditPage.faq.id} className="bg-slate-950 py-16 sm:py-20">
          <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <h2 className="mb-8 text-3xl font-bold text-white sm:text-4xl">
              {websiteAuditPage.faq.title}
            </h2>
            <div className="space-y-4">
              {websiteAuditPage.faq.items.map((item) => (
        ...
        <section className="bg-slate-950 px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          <div className="mx-auto max-w-5xl rounded-[2rem] border border-indigo-500/25 bg-gradient-to-br from-indigo-600/18 to-slate-900 p-6 shadow-2xl shadow-indigo-950/20 sm:p-8 lg:p-10">
            ...
            <h2 className="max-w-3xl text-3xl font-bold text-white sm:text-4xl">
              {websiteAuditPage.finalCta.title}
            </h2>
            <p className="mt-4 max-w-3xl text-base leading-relaxed text-indigo-100/85 sm:text-lg">
              {websiteAuditPage.finalCta.description}
            </p>
            <a
              href={websiteAuditPage.finalCta.buttonHref}
            ...
            >
              {websiteAuditPage.finalCta.buttonLabel}
            </a>
          </div>
        </section>

        <Contact defaultProjectType={WEBSITE_AUDIT_PROJECT_TYPE} />
      </main>
      <Footer />
    </div>
  );
