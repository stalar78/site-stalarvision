...
import { consentPersonalDataPage } from '@/data/legal';
import { profile } from '@/data/profile';
import { applyDocumentMeta } from '@/lib/meta';

export default function ConsentPersonalData() {
  useEffect(() => {
    applyDocumentMeta({
      lang: profile.seo.htmlLang,
      title: consentPersonalDataPage.seo.title,
      description: consentPersonalDataPage.seo.description,
      ogTitle: consentPersonalDataPage.seo.title,
      ogDescription: consentPersonalDataPage.seo.description,
      ogType: profile.seo.ogType,
      ogLocale: profile.seo.ogLocale,
      ogSiteName: profile.brand.fullName,
      ogUrl: `${profile.seo.siteUrl}/consent-personal-data/`,
      ogImage: profile.seo.ogImageUrl,
      canonical: `${profile.seo.siteUrl}/consent-personal-data/`,
      twitterCard: profile.seo.twitterCard,
      twitterTitle: consentPersonalDataPage.seo.title,
      twitterDescription: consentPersonalDataPage.seo.description,
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-10 sm:mb-12">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/20 bg-indigo-500/10 px-3 py-1 text-xs font-medium text-indigo-400 sm:mb-6 sm:text-sm">
              <ShieldCheck size={14} />
              <span>{consentPersonalDataPage.hero.eyebrow}</span>
            </div>
            <h1 className="mb-5 text-3xl font-extrabold leading-tight text-white sm:mb-6 sm:text-4xl md:text-6xl">
              {consentPersonalDataPage.hero.title}
            </h1>
            <p className="max-w-3xl text-base leading-relaxed text-slate-400 sm:text-lg">
              {consentPersonalDataPage.hero.description}
            </p>
            <p className="mt-4 text-sm leading-relaxed text-slate-500">
              Версия согласия: {consentPersonalDataPage.version}
            </p>
          </div>

          <div className="grid gap-6">
            {consentPersonalDataPage.sections.map((section, index) => (
              <section
                key={section.id}
                id={section.id}
