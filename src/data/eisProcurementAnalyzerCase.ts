export const eisProcurementAnalyzerCase = {
  ...
  developmentPerspective: {
    ...
    eyebrow: 'Перспективы развития',
    title: 'Как можно развивать систему',
    description:
      'Текущий публичный кейс описывает реализованный pipeline анализа закупочной документации. Отдельно можно спланировать следующий слой развития вокруг мониторинга, рабочего кабинета, истории изменений, настраиваемой аналитики и интеграций.',
    introduction:
      'Такие направления имеют смысл, когда нужно не только разово проанализировать процедуры, но и встроить сбор, проверку и evidence-backed выводы в более регулярный рабочий контур.',
    groups: [
      {
        title: 'Мониторинг и уведомления',
        items: [
          'регулярный запуск сбора по расписанию;',
          'сохранённые поисковые профили;',
          'уведомления о новых подходящих закупках;',
          'контроль изменений карточек и документации;',
      ...
      },
      {
        ...
        items: [
          'веб-интерфейс со списком найденных закупок;',
          'фильтры, статусы и ручные пометки;',
          'просмотр evidence и источника каждого извлечённого значения;',
          'очередь закупок, требующих ручной проверки;',
          'управление правилами и профилями анализа.',
        ],
      },
      {
        title: 'Сравнение и история',
        items: [
          'сравнение редакций документации;',
          'история изменений закупки;',
          'сопоставление первоначальных и итоговых условий;',
          'накопление статистики по категориям, заказчикам и результатам;',
          'повторное использование проверенных правил анализа.',
        ],
      },
      {
        title: 'Настраиваемая аналитика',
        items: [
          'собственные критерии технической пригодности;',
          'разные модели рекомендаций для разных типов бизнеса;',
          'дополнительные показатели полноты и надёжности;',
          'собственные классификаторы документов;',
          'адаптированные отчёты и форматы выгрузки.',
        ],
      },
      {
        title: 'Интеграции',
        items: [
          'API и webhooks;',
          'обмен данными с CRM, ERP и внутренними системами;',
          'загрузка результатов в корпоративные хранилища;',
          'автоматическое создание задач для ответственных сотрудников;',
          'подключение внутренних справочников, ограничений и правил компании.',
        ],
      },
    ],
    sourceAdaptation: {
      title: 'Адаптация к другим площадкам',
      description:
        'Аналитическое ядро — классификация документов, evidence, контроль качества и отдельные уровни решения — можно переиспользовать при подключении других открытых источников. При этом для каждой площадки требуется отдельный адаптер сбора данных.',
      notes: [
        'структура страниц, API и способы получения документов у разных источников различаются;',
        'доступность интеграции зависит от публичного доступа, авторизации, условий использования и технических ограничений площадки;',
        'селекторы и правила загрузки требуют отдельной разработки и сопровождения;',
        'проект не предполагает обход CAPTCHA, закрытой авторизации или защитных механизмов;',
        'возможность адаптации оценивается отдельно для каждого источника.',
      ],
    },
    disclaimer:
      'Это направления возможного развития и адаптации проекта, а не функции текущей публичной версии. Объём работ для новой площадки зависит от способа доступа к данным, API, авторизации, структуры документов и ограничений источника.',
  },
  repository: {
    id: 'repository',
