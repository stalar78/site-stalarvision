...
- production deploy и ручной production QA ещё не выполнены.
...
1. Выполнить безопасный production deploy commit `6fe648fb3d037390773458e2e8f581fa7dcc4b51`.
2. Проверить новый маршрут напрямую и через SPA-навигацию на desktop и mobile.
3. Проверить title, description, canonical, Open Graph, JSON-LD, sitemap, форму с контекстом `Веб-приложение / личный кабинет`, Console и Network.
4. Проверить редирект URL без завершающего слеша на канонический URL со слешем; при его отсутствии подготовить отдельную точечную Nginx-правку после анализа текущего server block.
5. После успешного production QA отправить только новый URL на переобход в Яндекс Вебмастере и затем дождаться обработки.
6. Продолжать использовать `contact_form_success` и именованные контактные цели как основные цели конверсии.
7. Следующий growth-этап выбирать только после завершения deploy и QA новой коммерческой страницы; tooling upgrade и дальнейшая полировка Lighthouse не являются текущим приоритетом.
