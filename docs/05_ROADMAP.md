...
3. Диагностически оценить предупреждение Vite о chunk больше 500 kB: определить состав основного bundle и реальную пользу route-level code splitting без преждевременной переделки.
4. Tooling upgrade Vite/esbuild/PostCSS/Babel проводить отдельно, без `npm audit fix --force`.
5. Следующий реальный проект для публичного GitHub или витрины выбирать только после отдельного security/fact audit.
