...
2. Продолжать использовать `contact_form_success` и именованные контактные цели как основные цели конверсии.
3. Исправить неблокирующую семантическую ошибку вложенного `<section>` в Hero страницы `/razrabotka-sajta/` отдельным минимальным проходом.
4. После этого оценить необходимость code splitting для предупреждения о chunk больше 500 kB.
5. Tooling upgrade Vite/esbuild/PostCSS/Babel проводить отдельно, без `npm audit fix --force`.
6. Следующий реальный проект для публичного GitHub или витрины выбирать только после отдельного security/fact audit.
